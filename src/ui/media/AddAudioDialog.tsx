...
export function AddAudioDialog({ open, onClose }: Props) {
  ...
  const addExisting = (assetId: string) => {
    ...
    if (!track) return
    addClip(assetId, track.id)
    setNotice({ kind: 'ok', text: 'Added to the timeline' })
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 p-4 backdrop-blur-sm">
      <div className="flex max-h-[90svh] w-full max-w-md flex-col overflow-hidden rounded-2xl border bg-card shadow-2xl">
        <div className="flex items-center gap-2 border-b px-4 py-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-muted-foreground text-[11px]">Import or add audio to the timeline</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          ...
          >
            ...
            {busy ? 'Importing…' : 'Import audio file'}
          </Button>
