...
function buildHarness(nonce: string): string {
  ...
  const parts = [
    ...
    '(function () {',
    '  var W = __W__, H = __H__;',
    '  var canvas = null, ctx = null, gl = null;',
    '  var isWebGL = false;',
    '  var toParent = function (msg) { parent.postMessage(msg, "*"); };',
    '  function ensure() {',
    '    if (canvas) return;',
    '    canvas = new OffscreenCanvas(W, H);',
    '    if (typeof window.__WEBGL === "function" || typeof window.__WEBGL_INIT === "function") {',
    '      isWebGL = true;',
    '      gl = canvas.getContext("webgl2", { preserveDrawingBuffer: true, alpha: true, antialias: true, powerPreference: "high-performance" }) || canvas.getContext("webgl", { preserveDrawingBuffer: true, alpha: true, antialias: true, powerPreference: "high-performance" });',
    '      if (typeof window.__WEBGL_INIT === "function") {',
    '        try { window.__WEBGL_INIT(gl, W, H); } catch (e) {',
    ...
    '      ensure();',
    '      if (isWebGL && gl) {',
    '        if (typeof window.__WEBGL === "function") {',
    '          var r = window.__WEBGL(gl, d.t, W, H);',
