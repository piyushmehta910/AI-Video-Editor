...
const PRESET_OPTIONS: PresetOption[] = [
  {
    ...
    aspectRatio: '16:9',
    subtitle: 'Standard landscape 16:9 video',
    width: 1920,
    height: 1080,
    icon: Monitor,
    badge: 'Popular',
  },
  {
    ...
    aspectRatio: '9:16',
    subtitle: 'Full vertical 9:16 mobile format',
    width: 1080,
    height: 1920,
    icon: Smartphone,
    badge: 'Viral',
  },
  {
    ...
    aspectRatio: '1:1',
    subtitle: '1:1 feed square format',
    width: 1080,
    height: 1080,
    icon: Square,
  },
  {
    ...
    aspectRatio: '4:5',
    subtitle: '4:5 tall social feed post',
    width: 1080,
    height: 1350,
    icon: LayoutGrid,
  },
  {
    ...
    aspectRatio: '21:9',
    subtitle: '21:9 anamorphic widescreen',
    width: 2560,
    height: 1080,
    icon: Film,
  },
...
const RESOLUTION_SCALES: Record<string, { label: string; scale: number; desc: string }> = {
  '720p': { label: '720p HD', scale: 0.6667, desc: 'Fast rendering & low memory' },
  '1080p': { label: '1080p Full HD', scale: 1, desc: 'Recommended standard' },
  '1440p': { label: '1440p 2K', scale: 1.3333, desc: 'Crisp high resolution' },
  '4k': { label: '4K Ultra HD', scale: 2, desc: 'Maximum quality' },
}

const FPS_OPTIONS = [
  { value: 24, label: '24 fps', desc: 'Cinematic Film' },
  { value: 30, label: '30 fps', desc: 'Standard Web & Social' },
  { value: 60, label: '60 fps', desc: 'Smooth / High Motion' },
]
...
export function NewProjectDialog({ open, onClose }: NewProjectDialogProps) {
  ...
  const finalWidth = Math.round(selectedPreset.width * quality.scale)
  const finalHeight = Math.round(selectedPreset.height * quality.scale)
  ...
  return (
    <div className="fixed inset-0 z-[10100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
      <div
        className="w-full max-w-2xl rounded-2xl border border-border/80 bg-card p-5 sm:p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150 max-h-[92vh] overflow-y-auto"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Configure your canvas aspect ratio, resolution, and platform format presets.
              </p>
            </div>
          </div>
          <button
        ...
        {/* Aspect Ratio & Platform Presets Grid */}
        <div className="space-y-2">
          <label className="text-xs font-semibold text-foreground flex items-center justify-between">
            <span>Platform & Aspect Ratio Presets</span>
            <span className="text-[10px] font-mono text-violet-500 font-bold">
              {selectedPreset.aspectRatio} ({finalWidth}×{finalHeight})
            </span>
          </label>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 mt-1 text-[10px] text-muted-foreground font-mono">
                    <span className="font-bold text-foreground/80">{preset.aspectRatio}</span>
                    <span>•</span>
        ...
        {/* Quality & Resolution + FPS in 2 columns */}
              ...
              {Object.entries(RESOLUTION_SCALES).map(([key, info]) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'px-2.5 py-2 rounded-lg border text-left text-xs font-semibold transition',
                      isSelected
              ...
              {FPS_OPTIONS.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'px-2 py-2 rounded-lg border text-center text-xs font-semibold transition flex flex-col items-center justify-center gap-0.5',
                      isSelected
                        ? 'border-violet-600 bg-violet-500/10 text-violet-600 dark:text-violet-300 font-bold'
                        : 'border-border/60 bg-muted/15 text-muted-foreground hover:bg-muted/30 hover:text-foreground',
                    )}
                  >
                    ...
                    <span className="text-[9px] text-muted-foreground/80 leading-none truncate max-w-full">
                      {f.desc.split(' ')[0]}
                    </span>
                  </button>
                )
        ...
        {/* Live Project Summary Box */}
        <div className="rounded-xl border border-border/70 bg-muted/20 p-3 flex items-center justify-between text-xs">
          <div className="space-y-0.5">
            <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold">
              Project Specification
            </span>
            <div className="flex items-center gap-2 font-mono text-[11px] text-foreground font-semibold">
              <span>{selectedPreset.aspectRatio}</span>
              <span>•</span>
          ...
          <span className="text-[10px] text-muted-foreground max-w-[200px] text-right hidden sm:inline">
            Media library assets are preserved safely.
          </span>
        </div>

        {/* Actions */}
        <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-border/60">
          <Button variant="outline" size="sm" onClick={onClose} className="rounded-lg px-4 text-xs font-semibold">
            Cancel
          </Button>
          <Button
            size="sm"
            onClick={handleCreate}
            className="rounded-lg px-5 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-bold text-xs shadow-md shadow-violet-500/20"
          >
